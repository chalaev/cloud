...
(Probably I should rename =junk-extensions= → =black-extensions= and =ignored-dirs= → =black-dirs= for compatibility with [[https://github.com/chalaev/el-backup][el-backup]].)
#+BEGIN_SRC emacs-lisp :tangle generated/variables.el
...
(do-make))
#+END_SRC
←  end of =(clog-if-failed (directory-lock...))=
...
The above function =DRF= needs =rename-directory=:
#+BEGIN_SRC emacs-lisp :tangle generated/functions.el
