...
Similarly to how they do it for ~JavaScript~, let me call the following functions "dynamic variables":
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
...
2. in local file named =(local/hosts)= ← host list,
3. in local file named =(local/actions)= ← actions, and
4. on the server, in a file named .

From time to time (e.g., when we =M-x cloud-sync=)  we synchronize local information
[stored in =(local/hosts)=, =(local/actions)=, and =(local/files)= ] with the remote file whose (secret) name is stored in =remote/files=:
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
...
*** hosts and actions
The files named =(local/hosts)= and  =(local/actions)= contain ~cloud-hosts~ and ~remote-actions~:
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
(defun local/hosts() (concat (local/) "hosts"))
(defun local/actions() (concat (local/) "actions"))
#+END_SRC
The content of ~file-DB~ is locally stored in the file named =(local/files)=:
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
(defun local/files() (concat (local/) "files"))
(defun local/all() (concat (local/) "all"))
#+END_SRC

Sometimes we cannot (when there is not internet connection) to synchronize
our local variables and local files with the cloud,
or there is just no need to do that — when our host was the last one who updated the cloud.

This is determined by the function
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
...
#+END_SRC
returning ~t~ if there were no updates from other hosts.

Here are some function that we need to synchronize all this information:
*** printing
**** hosts
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
(defun print-hosts()
...
** Other variables
~Makefile~ merges ~{0,macros,functions,logging,1,2}.el~ together with ~main.el~ into ~cloud.el~, which is launched from =~/.emacs=.
...
#+END_SRC
← I created this variable to prevent recovering them from the cloud during the next =(cloud-sync)=.
#+BEGIN_SRC emacs-lisp :tangle generated/main.el :shebang ";;; -*- mode: Emacs-Lisp;  lexical-binding: t; -*-"
...
** Writing remote file DB and understanding its format.
...
** Parsing
*** list of host names
The function =read-fileDB*= (returns =nil=) reads (previously decrypted) [[Host/Action/File information][host/action/file information]]:
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
...
which can be fed to GNU make as =make --jobs=N -f cloud.mk=, where ~jobs~ parameter is the (configurable) number ~N~ of CPU cores.
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
(forward-line)
(needs ((CDFs
#+END_SRC
...
#+END_SRC
We save (i) host names  to =~/.emacs.d/cloud/`hostname`/hosts=
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
(with-temp-file (local/hosts) (print-hosts))
#+END_SRC
and (ii) actions to =~/.emacs.d/cloud/`hostname`/actions=:
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
...
#+END_SRC
Let us see if there are any pending actions that we could not write to the cloud last time:
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
...
If there were no recent cloud updated by other hosts
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
(if (no-new-updates-p)
#+END_SRC
we normally don't need to read the cloud, except when ~file-DB~ is empty (this happens when we are launching =cloud-sync= at start):
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
(unless file-DB (read-fileDB))
#+END_SRC
If another host has updated the cloud, we have to examine the updates:
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
...
#+END_SRC
Tell other hosts that our host was the last to make changes on the cloud:
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
...
may be I will just write an empty line to the file without writing timestamps to it – when the code finally becomes stable.
...
Let us save file information to the local file and then copy it (encrypted) to the cloud:
#+BEGIN_SRC emacs-lisp :tangle generated/main.el
(write-all (local/all))
