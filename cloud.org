...
(rm (untilde(cloud-mk)))
  (reset-Makefile))))
#+END_SRC
← Probably I should use =start-process= in the future; it is more versatile for elisp than (interactive) =shell-command= or =async-shell-command=.
(See [[file:learn.org][learn.org]] for =call-process=.)
...
#+END_SRC
←  end of =(if-failed (directory-lock...))=
